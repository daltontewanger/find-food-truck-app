    ...
    useEffect(() => {
        const fetchFoodTruckDetails = async () => {
            ...
            try {
                ...
                const response = await axios.get('http://localhost:5000/api/foodtruck/details', {
                    headers: {
                ...
                if (response.status === 200) {
                    const { name, description, foodType, menuLink, schedules } = response.data;
                    setName(name || '');
                ...
                alert('Failed to fetch food truck details. Please try again.');
            } finally {
                setLoading(false);
            }
        };
